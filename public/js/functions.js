var host   = '127.0.0.1';//'192.168.0.82';
var port   =  '3000';
...
//Con esto cuando mando muchisimos mensajes 
//lo que hago es si hacen click en el scroll quito la
//animacion por 2 segundos y la vuelvo a colocar
var clicked = false;
var timer   = null;
...
function actualizarDocumentTitle(){    
    $(document).attr("title", "Chat ("+unreadMessagesCount+")");
}
    ...
    socket.on("refreshChat", function(action, message)
    {
        ...
        else if(action == "msg")
        {
            ...
            if (message.indexOf('Yo : ')==0){
                $("#chatMsgs").append("<p class='col-md-12 form-control'>" + message + "</p>");
            }
            else{
                unreadMessagesCount++;
